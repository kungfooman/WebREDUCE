...
class TempFuncs {
  ...
  inputDivInsert(text) {
    ...
    if (insertionNode === lastInput) {
      // Whole node rather than content selected:
      lastInput.textContent = text;
    } else {
      // Content selected:
      if (!(lastInput.contains(insertionNode) && lastInput.contains(selProps.focusNode))) {
      ...
      lastInput.textContent = cont.substr(0, selBeg) + text + cont.substr(selEnd);
    }
    const index = text.search(/\bws\b/), selection = getSelection();
    if (index >= 0) {
      // Select ws if available within inserted text:
      selection.setBaseAndExtent(lastInput.firstChild, selBeg + index, lastInput.firstChild, selBeg + index + 2);
    } else {
      // Otherwise, set caret to end of insertion:
      selection.collapse(lastInput.firstChild, selBeg + text.length);
    }
  }
}
/**
